...
export default function Home() {
  const [urlList, setUrlList] = useState<UrlEntity[]>([]);
  ...
  const createShortedUrl = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const longetUrl = data.longestUrl;

    try {
      const res = await fetch('http://localhost:3000/post/newURl', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ longetUrl })
      });
    ...
    <>
      <div className='w-full h-full flex justify-center p-10'>
        <div className='w-full flex flex-col items-center gap-5'>
          <h1 className='text-white font-semibold sm:text-6xl mb-10'>
            ShortenUrl
            <span className='mt-10 text-sm text-white/90 font-normal'>
              by Mohammed Hilali
            </span>
          </h1>
          ...
          <form onSubmit={createShortedUrl} className='m-5 text-center max-w-200 w-full'>
            <label htmlFor="url" className='text-white w-full text-3xl font-medium'>
            ...
            <div className='flex items-center w-full mt-2'>
              <input
                ...
                id='url'
                name='longestUrl'
                className='py-3 px-5 focus:outline-3 bg-white focus:outline-blue-700 w-full text-2xl text-black rounded-tl-xl rounded-bl-xl'
          ...
          <div>
            ...
            {!urlList.length ? (
              <p className='text-white text-center font-medium'>No urls yet :(</p>
            ) : (
          ...
          <Link to="/urls"
            className='bg-blue-700 text-2xl font-medium py-3 px-5 text-white active:bg-blue-900 rounded-xl'>
            See more
          </Link>
        </div>
      </div>
